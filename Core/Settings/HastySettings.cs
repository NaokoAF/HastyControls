...
public static class HastySettings
{
	...
	public class GeneralDisableAbilitiesInSafeZonesSetting() : HastyBoolSetting(
		Category, "Disable Abilities in Safe Zones",
		"Prevent accidental usage of abilities in the shop and healing areas.",
		true
	);

	public class GamepadSensitivityRatioSetting() : HastyFloatSetting(
		Category, "Gamepad Sensitivity Ratio",
		"Vertical sensitivity multiplier. 0.75 means vertical sensitivity is 75% slower than horizontal.",
		0f, 5f, 1f
	);

	public class GamepadPowerCurveSetting() : HastyFloatSetting(
		Category, "Gamepad Power Curve",
	...
	public class GamepadDeadzonesSetting() : HastyFloatSetting(
		Category, "Gamepad Deadzones",
		"Adjust deadzones to combat stick drift.",
		0.01f, 1f, 0.125f
	);

	public class GyroSensitivitySetting() : HastyFloatSetting(
		Category, "Gyro Sensitivity",
		"Gyro sensitivity. 2 means a 90 degree turn rotates the camera 180 degrees.",
		0f, 10f, 3f
	);

	public class GyroSensitivityRatioSetting() : HastyFloatSetting(
		Category, "Gyro Sensitivity Ratio",
		"Vertical sensitivity multiplier. 0.75 means vertical sensitivity is 75% slower than horizontal.",
		0f, 5f, 0.75f
	);

	public class GyroDisableWhenWalkingSetting() : HastyBoolSetting(
		Category, "Gyro Disable when Walking",
		"Disable gyro while slow walking.",
		false
	);

	public class GyroSpaceSetting() : HastyEnumSetting<GyroSpace>(
		Category, "Gyro Space",
		"Algorithm used to convert real world movements to the in game camera.",
		GyroSpace.PlayerTurn,
		[
			"Local Yaw", // LocalYaw
			"Local Roll", // LocalRoll
			"Player Turn", // PlayerTurn
			"Player Lean" // PlayerLean
		]
	);

	public class GyroOrientationSetting() : HastyEnumSetting<GyroOrientation>(
		Category, "Gyro Orientation",
		"Fixes devices with unusual orientations.",
		GyroOrientation.Auto,
		[
			"Auto", // Auto
			"Normal", // Normal
			"Steam Deck", // Deck
	...
	public class GyroButtonModeSetting() : HastyEnumSetting<GyroButtonMode>(
		Category, "Gyro Modifier Mode",
		"Behavior of gyro modifier button.",
		GyroButtonMode.Off,
		[
			"Disable while held", // Off
			"Enable while held", // On
			"Toggle when pressed", // Toggle
			"Recenter when pressed", // Recenter
			"Recenter and disable while held" // RecenterAndOff
		]
	);

	public class GyroUseTouchpadAsModifier() : HastyBoolSetting(
		Category, "Gyro Use Touchpad as Modifier",
		"Interpret touchpad touches as an additional gyro modifier button.",
		true
	);

	public class GyroTighteningSetting() : HastyFloatSetting(
		Category, "Gyro Tightening",
		"Soft gyro deadzone to reduce the effects of shaky hands.",
		0f, 15f, 3f
	);

	public class GyroSmoothingThresholdSetting() : HastyFloatSetting(
		Category, "Gyro Smoothing Threshold",
		"Threshold below which gyro rotations are smoothed.",
		0f, 300f, 35f
	);

	public class GyroSmoothingTimeSetting() : HastyFloatSetting(
		Category, "Gyro Smoothing Time",
		"Amount of time gyro is smoothed for.",
		0f, 0.5f, 0.1f
	);

	public class GyroInvertXSetting() : HastyBoolSetting(
		Category, "Gyro Invert X",
		"Invert gyro X axis (Horizontal).",
		false
	);

	public class GyroInvertYSetting() : HastyBoolSetting(
		Category, "Gyro Invert Y",
		"Invert gyro Y axis (Vertical).",
		false
	);

	public class AutoLookHorSpeedSetting() : HastyFloatSetting(
		Category, "AutoLook Horizontal Speed",
		"How fast the camera faces the direction of the left stick.",
		0f, 10f, 1f
	);

	public class AutoLookHorStrengthSetting() : HastyFloatSetting(
		Category, "AutoLook Horizontal Strength",
		"How strongly the camera faces the direction of the left stick.",
		0f, 10f, 1f
	);

	public class AutoLookVerSpeedSetting() : HastyFloatSetting(
		Category, "AutoLook Vertical Speed",
		"How fast the camera follows your vertical velocity.",
		0f, 10f, 1f
	);

	public class AutoLookVerStrengthSetting() : HastyFloatSetting(
		Category, "AutoLook Vertical Strength",
		"How strongly the camera follows your vertical velocity.",
		0f, 10f, 0f
	);

	public class AutoLookVerBaseAngleSetting() : HastyFloatSetting(
		Category, "AutoLook Vertical Base Angle",
		"Default vertical angle while touching the ground. Higher values tilt the camera downward.",
		-5f, 5f, 1f
	);

	public class RumbleEnabledLegacySetting() : HastyBoolSetting(
		Category, "Rumble Enabled",
		"Enable HastyControl's old rumble implementation (Before Patch 1.7.a)",
		false
	);

	public class RumbleIntensitySetting() : HastyFloatSetting(
		Category, "Rumble Intensity",
		"Overall rumble intensity.",
		0f, 5f, 1f
	);

	public class RumbleOnDamageSetting() : HastyFloatSetting(
		Category, "Rumble on Damage Intensity",
		"Rumble intensity when taking damage.",
		0f, 5f, 1f
	);

	public class RumbleOnLandSetting() : HastyFloatSetting(
		Category, "Rumble on Land Intensity",
		"Rumble intensity when landing. Different landing grades have different rumble properties.",
		0f, 5f, 1f
	);

	public class RumbleOnFastRunSetting() : HastyFloatSetting(
		Category, "Rumble on Fast Run Intensity",
		"Rumble intensity when charging up fast run.",
		0f, 5f, 1f
	);

	public class RumbleOnSparkPickupSetting() : HastyFloatSetting(
		Category, "Rumble on Spark Pickup Intensity",
		"Rumble intensity when picking up sparks.",
		0f, 5f, 1f
	);

	public class RumbleOnBoostRingSetting() : HastyFloatSetting(
		Category, "Rumble on Boost Ring Intensity",
		"Rumble intensity when going through a boost ring.",
		0f, 5f, 1f
	);

	public class RumbleOnBoardBoostSetting() : HastyFloatSetting(
		Category, "Rumble on Board Boost Intensity",
		"Rumble intensity when board boosting.",
		0f, 5f, 1f
	);

	public class RumbleOnGrappleSetting() : HastyFloatSetting(
		Category, "Rumble on Grapple Intensity",
		"Rumble intensity when activating the grappling hook (Heir's Javelin).",
		0f, 5f, 1f
	);

	public class RumbleOnFlySetting() : HastyFloatSetting(
		Category, "Rumble on Poncho Intensity",
		"Rumble intensity when activating the poncho (Sage's Cowl).",
		0f, 5f, 1f
	);

	public class ResetSettingsSetting() : HastyButtonSetting(
		Category, "Revert Settings to Defaults",
		"Resets all HastyControls settings to defaults.",
		"Reset", HastySettings.Reset
	);
	...
	private static InputAction AddInputAction(Guid id, string name, string? path, string? actionMap)
	{
		...
		// check if the action name isn't in use
		if (map.FindAction(actionName) != null) throw new InvalidOperationException($"Input action {actionName} already exists.");
	...
	{
		public static readonly T Setting = new();
	}
}
